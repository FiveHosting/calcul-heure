...
function showApp() {
    ...
    document.getElementById('headerRole').textContent = String(currentUser.role).toLowerCase() === 'admin' ? 'Administrateur' : 'Utilisateur';
    updateAdminButtonVisibility();
    syncMonthPicker();
    loadEntries();
    loadMonthlyStats();
...
function renderSummaryCards(container, cards) {
    ...
    cards.forEach((card) => {
        ...
        if (card.subvalue) {
            el.appendChild(createTextElement('div', `summary-subvalue ${card.subClass || 'neutral'}`, card.subvalue));
        }
...
function aggregateEntriesForMonth(entries, month) {
    ...
    return filtered.reduce((acc, entry) => {
        ...
        return acc;
    }, { filtered: [], count: 0, hours: 0, salary: 0, byDay: {} });
}
...
function diffPercent(current, previous) {
    ...
    const sign = pct > 0 ? '+' : '';
    return { text: `${sign}${pct.toFixed(1)}% vs mois précédent`, className: pct >= 0 ? 'positive' : 'negative' };
}
...
function drawDonutChart(canvasId, data) {
    ...
    let legendY = cssHeight - 54;
    data.forEach((item, index) => {
        const x = 22 + index * 170;
        ctx.fillStyle = item.color;
    ...
    try {
        ...
        currentMonthData
            ...
            .forEach((entry) => {
                ...
                actions.appendChild(createIconButton('btn-delete', 'fas fa-trash', 'Supprimer', () => deleteEntry(entry.id)));
...
async function loadMonthlyStats() {
    try {
        ...
        document.getElementById('hoursChartTitle').textContent = getMonthLabel(selectedMonth);
        document.getElementById('salaryChartTitle').textContent = getMonthLabel(selectedMonth);

        renderSummaryCards(document.getElementById('monthlyStats'), [
            { label: 'Entrées', value: String(current.count), subvalue: getMonthLabel(selectedMonth), subClass: 'neutral' },
            { label: 'Heures', value: `${current.hours.toFixed(1)}h`, subvalue: `Moyenne ${(current.count ? current.hours / current.count : 0).toFixed(1)}h / entrée`, subClass: 'neutral' },
            { label: 'Salaire', value: formatMoney(current.salary), subvalue: `Moyenne ${formatMoney(current.count ? current.salary / current.count : 0)} / entrée`, subClass: 'neutral' }
        ]);
        ...
        renderSummaryCards(document.getElementById('comparisonStats'), [
            { label: 'Évolution des entrées', value: String(current.count - previous.count), subvalue: diffEntries.text, subClass: diffEntries.className },
            { label: 'Évolution des heures', value: `${(current.hours - previous.hours).toFixed(1)}h`, subvalue: diffHours.text, subClass: diffHours.className },
            { label: 'Évolution du salaire', value: formatMoney(current.salary - previous.salary), subvalue: diffSalary.text, subClass: diffSalary.className }
        ]);
    ...
    try {
        ...
        if (isMobile) {
            users.forEach((user) => {
                ...
                [['Entrées', String(user.total_entries || 0)], ['Heures', `${Number(user.total_hours || 0).toFixed(1)}h`], ['Salaire', formatMoney(user.total_salary || 0)]].forEach(([label, value]) => {
                ...
                actions.appendChild(createIconButton('btn-action', 'fas fa-user-tie', user.role === 'admin' ? 'Retirer admin' : 'Admin', () => changeUserRole(user.id, user.role, user.username)));
                if (user.id !== currentUser.id) {
                    actions.appendChild(createIconButton('btn-action-delete', 'fas fa-trash', 'Supprimer', () => deleteUser(user.id, user.username)));
                } else {
            ...
            users.forEach((user) => {
                ...
                roleTd.appendChild(createTextElement('span', `badge ${user.role === 'admin' ? 'badge-admin' : 'badge-user'}`, user.role));
                ...
                actionsTd.appendChild(createIconButton('btn-action', 'fas fa-user-tie', user.role === 'admin' ? 'Retirer admin' : 'Admin', () => changeUserRole(user.id, user.role, user.username)));
                if (user.id !== currentUser.id) {
                    actionsTd.appendChild(createIconButton('btn-action-delete', 'fas fa-trash', 'Supprimer', () => deleteUser(user.id, user.username)));
                } else {
        ...
        const topUsers = [...users].sort((a, b) => Number(b.total_salary || 0) - Number(a.total_salary || 0)).slice(0, 6);
        drawBarChart('adminSalaryChart', topUsers.map((u) => u.username.slice(0, 8)), topUsers.map((u) => Number(u.total_salary || 0)), 'rgba(34, 197, 94, 0.88)', (v) => `${v.toFixed(0)}€`);
    } catch (err) {
        showAlert(err.message, 'error');
    }
}
...
async function submitChangePassword() {
    ...
    try {
        await apiFetch('/auth/change-password', { method: 'POST', body: { oldPassword: oldPwd, newPassword: newPwd } });
...
function bindUIEvents() {
    ...
    if (registerToggle) registerToggle.addEventListener('click', () => switchAuthPanel('register'));

    const loginToggle = document.getElementById('loginToggle');
    if (loginToggle) loginToggle.addEventListener('click', () => switchAuthPanel('login'));

    const changePasswordBtn = document.getElementById('changePasswordBtn');
    if (changePasswordBtn) changePasswordBtn.addEventListener('click', changePassword);

    const exportPdfBtn = document.getElementById('exportPdfBtn');
    if (exportPdfBtn) exportPdfBtn.addEventListener('click', exportSelectedMonthToPdf);
    ...
    if (monthDisplayBtn && monthPicker) {
        monthDisplayBtn.addEventListener('click', () => {
    ...
    if (prevMonthBtn) prevMonthBtn.addEventListener('click', () => changeSelectedMonth(-1));

    const nextMonthBtn = document.getElementById('nextMonthBtn');
    if (nextMonthBtn) nextMonthBtn.addEventListener('click', () => changeSelectedMonth(1));
    ...
    if (closeChangePwdBtn) closeChangePwdBtn.addEventListener('click', closeChangePwdModal);

    const submitChangePwdBtn = document.getElementById('submitChangePwdBtn');
    if (submitChangePwdBtn) submitChangePwdBtn.addEventListener('click', submitChangePassword);
