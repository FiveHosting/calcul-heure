const APP_CONFIG = window.CALCUL_HEURES_CONFIG || {};
const IS_NATIVE_APP = Boolean(window.Capacitor?.isNativePlatform?.()) || ['capacitor:', 'file:'].includes(window.location.protocol);
const MOBILE_TOKEN_KEY = 'calcul_heures_mobile_token';
...
const configuredUrl = String(APP_CONFIG.apiBaseUrl || '').trim().replace(/\/+$/, '');
...
function rememberAuthPayload(data) {
if (data?.token) {
...
if (!API_URL) {
throw new Error('URL API mobile manquante. Configure public/js/mobile-config.js avant de générer l’app.');
}
...
if ((response.status === 401 || response.status === 403) && IS_NATIVE_APP) {
clearMobileToken();
}
...
window.CalculHeuresAds?.hideBanner?.();
...
loadEntries();
loadMonthlyStats();
window.CalculHeuresAds?.showBanner?.();
scrollToPageTop();
}
...
const firstConfirm = confirm('Supprimer définitivement votre compte et toutes vos entrées ? Cette action est irréversible.');
if (!firstConfirm) return;

const secondConfirm = confirm('Dernière confirmation : voulez-vous vraiment supprimer ce compte ?');
...
clientType: IS_NATIVE_APP ? 'mobile' : 'web'
}
});
...
if (IS_NATIVE_APP && !API_URL) {
showAlert('Configure l’URL API mobile dans public/js/mobile-config.js.', 'error');
return;
}
